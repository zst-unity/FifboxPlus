...
namespace ZSToolkit.Editor.PaletteGenerator
{
    public class PaletteGeneratorWindow : EditorWindow
    {
        ...
        [MenuItem("ZSToolkit/Palette Generator")]
        ...
        private void OnGUI()
        {
            ...
            if (!string.IsNullOrEmpty(_error))
            {
                ...
                EditorGUILayout.LabelField(_error, textStyle);
            }
            ...
            else EditorGUILayout.LabelField("No palette loaded.");
        ...
        private void GenerateLibrary()
        {
            ...
            foreach (var line in _loaded)
            {
                if (!line.StartsWith('#') && line != _loaded.First())
                {
                    ...
                    if (ColorUtility.TryParseHtmlString($"#{rgbHex.Last()}", out Color color))
                    {
                        colors.Add(color);
                    }
                    else
                    {
                        _error = $"failed to parse HEX color: #{rgbHex.Last()}";
                    }
                }
            }
